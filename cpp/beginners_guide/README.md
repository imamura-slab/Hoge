...
***
## [フレンド](./friend)
- 通常, クラスのprivateメンバに, クラス外からアクセスしようとすると, エラーになる. そこで, クラスの中にfriend指示子をつけた関数を宣言し, クラス外でその関数の定義を書くと, その関数からprivateメンバにアクセスできる.
```
class Hoge {
  friend int 関数名(Hoge &);
}
...
```
- フレンド関数と, フレンド関数をまとめたフレンドクラスがある.



***
## [動的メモリ割り当て](./alloc_mem)
- C言語の`malloc()関数`, `free()関数`も使用できるがC++では推奨されない
- C++では, `new 演算子`と`delete 演算子`を使用する
```
new type;
```
```
...
***
## [オーバーライド](./override)
- オーバーライドとは, `仮想関数`による動的なポリモーフィズムである. (オーバーロードはコンパイル時の静的なポリモーフィズム)

#### 仮想関数
- 基底クラスのメンバ関数を派生クラスで再定義することができる. このときの基底クラスのメンバ関数を`仮想関数`と呼ぶ 
```
virtual type function(arg-list);
```

#### 純粋仮想関数
- 再定義して使用することを前提とした仮想関数の場合(基底クラスで定義しても使わない場合)は`純粋仮想関数`として宣言する. 純粋仮想関数を持つクラスを`抽象クラス`と呼び, 抽象クラスはオブジェクトを作ることができない.
```
